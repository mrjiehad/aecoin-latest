...
app.use("/api/billplz/callback", express2.text({ type: "application/x-www-form-urlencoded" }));
...
app.use((req, res, next) => {
  ...
  express2.urlencoded({ extended: false })(req, res, next);
});
...
(async () => {
  ...
  if (process.env.NODE_ENV !== "production") {
    ...
    const distDir = path2.resolve(__dirname, "../../client/dist");
  ...
  server.listen({
    ...
    log2(`serving on port ${port} (NODE_ENV=${process.env.NODE_ENV || "development"})`);
  });
})();
